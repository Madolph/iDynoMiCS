...
public class MultigridSoluteTimeDependent 
{
	...
	/**
	 * constant value to determine, if a voxel has to be consideredfor calculation
	 */
	...
	public MultigridSoluteTimeDependent(SoluteGrid aSolute, MultigridSoluteTimeDependent relDiff, 
			              MultigridSoluteTimeDependent bLayer, double deltat, Simulator aSim, double sBulk, int nReac) 
	{
		mySim=aSim;
		...
		stepGrid = new SoluteGrid(_i, _j, _k, r, aSolute);
	 ...
	 * @param aSolute
	 */
	public MultigridSoluteTimeDependent(SoluteGrid aSolute, String gridName, double deltat) 
	{
		soluteName = gridName;
		_domain = aSolute.getDomain();
		realGrid = aSolute;
		//changed for cell-centered grids
	...
	{	
		/*
		 * this part is basically doing the same as the Thomas-algorithm
		 * for tridiagonal matrices, but we don't want to solve the system right away.
		 * Instead, we take the transformed matrix and do a second step, that
		 * is explained below
		 */
	...
	/**
	 * solves the Diffusion according to the implicit Crank-Nicolson-Method.
	 * The spatial Dimensions are independent and therefore solved separately, breaking the
	 * domain down in 1D-parts.
	 * 2D Matrices are made up for computation. These are tridiagonal Matrices, that are solved 
	 * with the a matrix-algorithm.
	 * This is the basically heart of our time-dependent-solver.
	 * 
	 ...
	 */
	public void implCrankNic(SoluteGrid conc, SoluteGrid step, double dt, 
			...
			for (int y=0;y<nJ+2;y++)
			{
				...
				else
				{
					...
					for (int z=0;z<nKi;z++)
					{
						...
						double sum=0.0;
						sum=sum+B[z][z]*conc.grid[he][wi][de];
						if (z<nKi-1)
							sum=sum+B[z+1][z]*conc.grid[he][wi][de+1];
						if (z>0)
							sum=sum+B[z-1][z]*conc.grid[he][wi][de-1];
						d[z]=sum;
					}
				...
				for (int z=0;z<nK+2;z++)
				{
					...
					else
					{
						if (_bLayer._conc.grid[x][y][z]>BLTHRESH || !refresh)
							step.grid[x][y][z]=conc.grid[x][y][z]+XYZ[x][y][z];
						else
							step.grid[x][y][z]=sBulk; // this is disabled if refresh is false
					}
				}
		
		step.refreshBoundary("conc");
	}
	...
	/**
	 * calculates a stable dt for the solute, according to the criterion for our Crank-
	 * Nicolson-Method, which is dt<h²/D
	 * 
	 * @param step the amount of time we want to solve (so dt does not get larger than our step)
	 */
	public void computeStableDt(double step) 
	{
		{
			// calculates our fine spatial resolution
			double h=(_referenceSystemSide)/
					(referenceIndex(_conc.getGridSizeI()
								,_conc.getGridSizeJ()
								,_conc.getGridSizeK()));
			
			LogFile.writeLogAlways("h: "+h+" / diff: "+realGrid.diffusivity);
			
			double maxTime=(0.2*h*h)/realGrid.diffusivity; // our stepping-criterion
			
			// computation of our stable timestep
			double stepAmount= Math.ceil(step/(maxTime));
			dt=step/stepAmount; //dt is computed to split the miniStep evenly
	 ...
	 */
	public double referenceIndex(int i, int j, int k) 
	{
		if (_nK > 1)
	...
	/**
	 * writes our conc-grid to our realGrid.
	 * conc-grid should be used by the solver internally, while the
	...
	/**
	 * sets our step-grid values as our new conc-grid
	 */
	public void stepToConc() 
	{
		MatrixOperations.copyValuesTo(_conc.grid, stepGrid.grid);
	}
