 ...
 * described by its dimensionality (2D or 3D)
 * 
 * The computation domain is an evenly spaced rectilinear grid 
 * described by its dimensionality (2D or 3D), its size, geometry 
 * and the behaviour at its boundaries. See Figure 1 of the Lardon et al paper (2011) 
 * for a good description on how this is divided into several regions - 
 * the support, the bulk, the biofilm matrix, and the diffusion boundary layer. 
 * 
 * @since June 2006
 * @version 1.2
 ...
 * Department of Engineering Sciences and Applied Mathematics, Northwestern University (USA)
 * @author Sónia Martins (SCM808@bham.ac.uk), 
 * Centre for Systems Biology, University of Birmingham (UK)
...
public class Domain implements IsComputationDomain 
{
	...
	/**
	 * Array to hold the X position that is the top of the boundary layer 
	 * in each of the Y and Z positions. Used for self-attach scenarios
	 */
	...
	/**
	 * The ratio between the carrier surface (the substratum on which the biofilm grows) and 
	 * the bulk compartment volume. The physical volume of the system that appears 
	 * in the simulation definition. In m2/m3.
	 */
	...
	/**
	 * \brief Creates a computation domain compartment object with attributes specified 
	 * in the protocol file
	 * 
	 * The computation domain is an evenly spaced rectilinear grid described by 
	 * its dimensionality (2D or 3D), its size, geometry and the behaviour at its boundaries. 
	 * See Figure 1 of the Lardon et al paper (2011) for a good description on how this is divided 
	 * into several regions - the support, the bulk, the biofilm matrix, 
	 * and the diffusion boundary layer. The definition within the computationDomain markup 
	 * of the protocol file notes how these regions are set up. 
	 * This constructor sets up each computation domain that is specified in the protocol file.
	 * 
	...
	public Domain(Simulator aSim, XMLParser cdRoot) 
	{
		...
		// Determine the size of this domain. If this is a chemostat, 
		// this is a set size and does not have to be read in.
		...
		// Note that the boundary layer is initialised with padding, then ignores it 
		// (and never calculates the padding)
		// For consistency (for the moment), the top of the boundary layer array 
		// will work in the same way - thus we will 
		...
		// Note this wasn't previously done, but with self attachment we need to know where 
		// the boundary layer is before any agents are added. 
		// The function below was part of the refreshBioFilmGrids method - 
		// this now exists independently and is called whenever these grids need to be refreshed
	...
	/**
	 * \brief Creates a solute or species grid and initialises the values within that grid. 
	 * Used to create boundary and biomass grids
	 * 
	 * Creates a solute or species grid and initialises the values within that grid. 
	 * Used to create boundary and biomass grids
	 * 
	 ...
	 * 
	 * This method should be used when you have finished to register all boundary conditions; 
	 * this function will populate a 3-D matrix. 
	 * Apply all boundaries one after one ; 
	 * a point is outside of the computational domain if it is declared outside by 
	 * at least one of the Boundary Conditions
	 * 
	 */
	public void applyAllBoundary() {
		...
		for (int i = 0; i<_domainGrid.getGridTotalSize(1); i++) 
			for (int j = 0; j<_domainGrid.getGridTotalSize(2); j++) 
				repeat: for (int k = 0; k<_domainGrid.getGridTotalSize(3); k++) 
				{
					dC.set(i-1, j-1, k-1);
					...
					for (AllBC aBC : _boundaryList) 
					{
						...
						if (_domainGrid.getValueAt(i, j, k)==-1) 
							continue repeat;
	...
	/**
     * \brief Test if a given location is outside a boundary. Used to detect 
     * the crossed boundary when moving an agent
     * 
     * Test if a given location is outside a boundary. 
	...
	/**
     * \brief Test if a given location is outside a boundary other than the bottom. 
     * Used in self-attach scenarios
     * 
     * Test if a given location is outside a boundary other than the bottom. 
     * For self-attachment, the simulation detects a swimming agent 
     * may have crossed the substratum boundary and will then assume that agent attaches. 
     * However we need to check that move has not crossed
     * any of the other boundaries, else that move is invalid. 
     * To do this, all boundaries are checked. If using the method above, y0z could 
     * still be returned and thus we end up in a loop. 
     * Thus this has had to be adapted so this cannot be returned
     * 
	...
	public AllBC testCrossedBoundarySelfAttach(ContinuousVector newLoc) {
		...
		for (int iB = 0; iB<_boundaryList.size(); iB++) 
			...
			// Added a check to not return y0z - 
			// we know this has been crossed as the cell has met the substratum. 
			// We are only interested here in checking the other 7 boundaries
			if (_boundaryList.get(iB).isOutside(newLoc)) 
	 ...
	 * 
	 * Creates an array list containing the 'i' coordinate of the 
	 * computation domain that is the top of the boundary layer. 
	 * Note that this is in the resolution specified by the 'resolution' parameter 
	 * in the computation domain section of the protocol file, 
	 * and this may need to be adjusted if being used in calculations where the resolution differs
	 */
	public void calculateTopOfBoundaryLayer() {
		for(int k=1;k<_boundaryLayer.getGridSizeK()+1;k++)		
			for(int j=1;j<_boundaryLayer.getGridSizeJ()+1;j++)
				...
				// and thus we are out of the boundary layer
	 ...
	 * 
	 * Method to refresh relative diffusivity and boundary layer grids to ensure 
	 * biomass updated this step is included. 
	...
	public void refreshBioFilmGrids() {
		...
		if(Simulator.isChemostat)
			...
			_boundaryLayer.refreshBoundary("bLayer");
			
			// Now calculate the positions that are at the top of the boundary layer
			calculateTopOfBoundaryLayer();
				
			_diffusivityGrid.refreshBoundary("relDiff");
			_biomassGrid.refreshBoundary("bLayer"); }
	}
	 ...
	 * In previous versions of iDynoMiCS this method could be found within refreshBioFilmGrids. 
	 * This has been moved here as, with the addition of self attachment, 
	 * this method needs to be called before agent initialisation. KA May 2013
	 */
	public void calculateComputationDomainGrids() {
		...
		/*LogFile.writeLogAlways("printing Diff...");
		for (int x=0;x<_diffusivityGrid.grid.length;x++)
			for (int y=0;y<_diffusivityGrid.grid[1].length;y++)
				for (int z=0;z<_diffusivityGrid.grid[1][1].length;z++)
					LogFile.writeLogAlways("Diff: "+_diffusivityGrid.grid[x][y][z]);
		LogFile.writeLogAlways("printed Diff...");*/
	}
			...
			for (_j = 1; _j<_nJ+1; _j++) 
				for (_k = 1; _k<_nK+1; _k++) 
				{
					...
					if ((v==1) && bdryHasFreeNbh()) 
						// add the location if it has biomass or is in the boundary layer (v==1) 
						// and if the neighboring points are free (not biomass or bdry layer)
	...
	/**
	 * \brief Creates a list of doubles with the heights of the biofilm/liquid interface. 
	 * Used for writing simulation statistics
	 * 
	 * Creates a list of doubles with the heights of the biofilm/liquid interface. 
	 * Used for writing simulation statistics. 
	 * This routine is very basic in that it just captures the overall height (x-position) 
	 * of each point from the nearest carrier, but for most cases it should be sufficient
	 * 
	...
	public Double[] getInterface() {
		...
		LinkedList<LocatedGroup> border = currentSim.agentGrid.getLevelSet().getBorder();
	...
	/**
	 * \brief Sets the value of a grid space in the boundary layer, 
	 * indicating whether the space is part of the boundary layer, or biomass is present
	 * 
	 * Sets the value of a grid space in the boundary layer, 
	 * indicating whether the space is part of the boundary layer, or biomass is present
	 *  
		...
		for (int i = (-nInterval); i<=nInterval; i++) 
		{
			...
			if ((n+i>=0)&(n+i<_nI)) 
			{
				...
				for (int j = (-mInterval); j<=mInterval; j++) 
					...
					{	
						// 2D case  - 
						if (_biomassGrid.grid[n+i][cyclicIndex(m+j,_nJ+2)][1]>0)  
							return 1; 
						if (_domainGrid.grid[n+i][cyclicIndex(m+j,_nJ+2)][1]==0)  
							return 1; 
					} 
					else 
					{
	 ...
	 * 
	 * For cyclic boundaries, returns the index of the grid space on the 
	 * opposite side of the boundary
	 * 
	 * @param val	The integer grid space to check
